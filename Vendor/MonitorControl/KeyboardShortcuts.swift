...
enum KeyboardShortcuts {
  ...
  private static let userDefaultsPrefix = "monitores.shortcuts."
  ...
  static func removeAllHandlers() {
    ...
    self.keyUpHandlers = [:]
    if let eventHandler = self.eventHandler { RemoveEventHandler(eventHandler) }
...
extension KeyboardShortcuts {
  final class RecorderCocoa: NSSearchField {
    ...
    init(for name: KeyboardShortcuts.Name) {
      ...
      isEditable = false
      isSelectable = false
      placeholderString = NSLocalizedString("Gravar atalho", comment: "Shortcut recorder placeholder")
      alignment = .center
    ...
    private func startRecording() {
      ...
      placeholderString = NSLocalizedString("Pressione o atalho", comment: "Shortcut recorder prompt")
      stringValue = ""
    ...
    private func handle(_ event: NSEvent) -> NSEvent? {
      ...
      if let menuItem = shortcut.takenByMainMenu {
        self.endRecording()
        NSAlert.showModal(for: window, title: String(format: NSLocalizedString("Este atalho já é usado pelo item de menu \"%@\".", comment: "Shortcut conflict; menu item title"), menuItem.title))
      ...
      guard !shortcut.isTakenBySystem else {
        self.endRecording()
        NSAlert.showModal(for: window, title: NSLocalizedString("Este atalho já é usado pelo sistema.", comment: "Shortcut conflict"), message: NSLocalizedString("Altere os atalhos nos ajustes de teclado do macOS.", comment: "Shortcut conflict guidance"))
        if window.isKeyWindow {
          window.makeFirstResponder(self)
        }
        return nil
